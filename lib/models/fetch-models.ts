...
export async function fetchOpenAICompatibleModels(): Promise<Model[]> {
  ...
  try {
    const baseUrl = process.env.OPENAI_COMPATIBLE_API_BASE_URL
    ...
    if (!response.ok) {
      console.warn('[ModelFetch] OpenAI Compatible API returned:', response.status)
      // Fallback to hardcoded DeepSeek models
      return [
        {
          id: 'deepseek-chat',
          name: 'DeepSeek Chat',

...
const screen = {
  getByRole: (role: string) => {
    const element = document.querySelector(`[role="${role}"], ${role}`)
    if (!element) throw new Error(`Unable to find element by role: ${role}`)
    return element
  },
  getAllByRole: (role: string) => {
    const elements = Array.from(document.querySelectorAll(`[role="${role}"], ${role}`))
    if (!elements.length) throw new Error(`Unable to find elements by role: ${role}`)
    return elements
  },
  ...
  getAllByTestId: (id: string) => {
    ...
    if (!elements.length) {
      throw new Error(`Unable to find elements by test id: ${id}`)
    }
    return elements
  },
  ...
  getByText: (text: string) => {
    ...
    if (!element) throw new Error(`Unable to find element by text: ${text}`)
    return element
  }
}
